...
@Component({
  ...
  standalone: true,
  imports: [PokemonListComponent, PokemonSearchComponent, AsyncPipe],
...
export class PokemonFrontPageComponent implements OnInit {
  ...
  caughtPokemons = !!this.savedPokemons
    ? JSON.parse(this.savedPokemons).map(
        (pokemon: PokemonModel) => pokemon.name,
      )
    : [];
      ...
      this.pokemonService
        ...
        .pipe(
          ...
          switchMap((pokemons: PokemonApiModel) => {
            const observables = pokemons.results.map(
              (pokemon: GenericAPIModel) =>
        ...
        .subscribe((pokemons: PokemonModel[]) => {
          pokemons.map(
            (pokemon: PokemonModel) =>
              (pokemon.caught = !!this.caughtPokemons.includes(pokemon.name)),
          );
